...
export class GroupManagementRefactor1776003840512 implements MigrationInterface {
    name = 'GroupManagementRefactor1776003840512'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`groups\` DROP FOREIGN KEY \`FK_89ce024cb71a24a02b09fb8f879\``);
        await queryRunner.query(`ALTER TABLE \`groups\` DROP COLUMN \`schedule\``);
        await queryRunner.query(`ALTER TABLE \`groups\` DROP COLUMN \`courseId\``);
        await queryRunner.query(`ALTER TABLE \`groups\` ADD \`description\` text NULL`);
        await queryRunner.query(`ALTER TABLE \`groups\` ADD \`type\` enum ('public', 'private') NOT NULL DEFAULT 'public'`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`groups\` ADD \`courseId\` int NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`groups\` ADD \`schedule\` text NULL DEFAULT 'NULL'`);
        await queryRunner.query(`ALTER TABLE \`groups\` ADD CONSTRAINT \`FK_89ce024cb71a24a02b09fb8f879\` FOREIGN KEY (\`courseId\`) REFERENCES \`courses\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    }
